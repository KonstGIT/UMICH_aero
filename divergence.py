...
def eff_stiff(U,rho,l,c,x_ea,x_ac,x_sp,k,kin_visc,profile="0012",pdfobj=-1): # effective stiffness
    S=l*c
    return 4*Ka(k,x_ea,x_sp)-S*cla(U,kin_visc,c,profile=profile,pdfobj=pdfobj)*.5*U*U*rho*(x_ea-x_ac)
...
def get_flutter():
    ...
    EV=np.zeros([len(U),2])
    for i in range(len(U)):
        EV[i,:]=flutter_static(U[i],kh,ka,S,m,Sa,Ia,e,c)
    return EV
    

# static
def flutter_static(U,Kh,Ka,S,m,Sa,Ia,e,c,kin_visc=15.11E-6,profile="0012",rho=1.22):
    # generate matrix
    dclda=cla(U,kin_visc,c)
    q=.5*rho*U**2
